...
<div class="container">
    ...
    <table class="table table-striped">
        ...
        <tr class="dataTableHeadingRow">
            ...
            <?php
/*
 * BOF - Product location product location heading
 */
            if (defined('PRODUCTS_LOCATION_PACKING_DISPLAY') && PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                ?>
            <th class="dataTableHeadingContent" align="left"><?php echo TABLE_HEADING_LOCATION; ?></th>
            <?php
            }
/*
 * EOF - Product location
 */
?>
        </tr>
        </thead>
        ...
        <?php
/*
 * BOF Product location sort
 */
        if (defined('PRODUCTS_LOCATION_PACKING_DISPLAY') && PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {

            $products_location=array();
            for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
                $products_location[$i]["order"] = $i;
                $products_location[$i]["model"] = $order->products[$i]['model'];
                $products_location_query = $db->Execute("SELECT IFNULL(products_location,'') AS products_location FROM " . TABLE_PRODUCTS_LOCATION . "
        								   WHERE products_id = '" . $order->products[$i]['id'] ."' LIMIT 1");
                $products_location[$i]["location"]=$products_location_query->fields['products_location'];
            }
            if (PRODUCTS_LOCATION_SORT_DISPLAY == 'true') $products_location_sort = array_multisort(array_column($products_location, 'location'), SORT_ASC, array_column($products_location, 'model'), SORT_ASC, $products_location);

        }
        for ($ii = 0, $n = sizeof($order->products); $ii < $n; $ii++) {
            
            if (defined('PRODUCTS_LOCATION_PACKING_DISPLAY') && PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                $i=$products_location[$ii]['order'];
            } else {
                $i=$ii;
            }
            /*
            ...
            <tr class="dataTableRow">
                ...
                <td class="dataTableContent">
                    ...
                    <?php
/*
 * BOF - Product location display location
 */
                      if (defined('PRODUCTS_LOCATION_PACKING_DISPLAY') && PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                    ?>
                <td class="dataTableContent" align="left" valign="top">
                    <?php echo  $products_location[$ii]["location"]; ?>
                </td>
                    <?php
                    }
// EOF - Product location
