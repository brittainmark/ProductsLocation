...
if ( isset($_GET['import']) ) {
	...
	if ($ep_supported_mods['ploc'] == true) { // Products Location Mod
		$default_these[] = 'v_products_location';
	}
  if (count($custom_fields) > 0) {
		foreach ($custom_fields as $field) {
			$default_these[] = 'v_'.$field;
		}
	}
...
// HERE ==========================>
				...
				if ($result == true) {
					...
					if ($ep_supported_mods['ploc'] == true) { // Products Location Mod
						$plquery = 'INSERT INTO '.TABLE_PRODUCTS_LOCATION.' SET
							products_id			= '.$v_products_id.',
							products_location	= '."'".$v_products_location."'";
						$plresult = ep_4_query($plquery);
						if ($plresult == false) {
							$query = 'DELETE '.TABLE_PRODUCTS.' WHERE products_id = '."'".$v_products_id."'";
							$result = ep_4_query($query);
        ...
        if ($ep_supported_mods['map'] == true) { // Manufacturer's Advertised Price
          ...
          $query .= "map_price = '".$v_map_price."',";
				...
				if ($result == true) {
					if ($ep_supported_mods['ploc'] == true) { // Products Location Mod
						$plquery = 'UPDATE '.TABLE_PRODUCTS_LOCATION.' SET
							products_id			= '.$v_products_id.',
							products_location	= '."'".$v_products_location."'".
							' WHERE (products_id = '."'".$v_products_id."')";
						$plresult = ep_4_query($plquery);
