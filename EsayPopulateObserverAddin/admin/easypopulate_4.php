...
////$ep_supported_mods['ploc'] = defined('TABLE_PRODUCTS_LOCATION');
...
////           echo 'Product Location mod:'.(($ep_supported_mods['ploc']) ? '<font color="green">TRUE</font>':"FALSE").'<br/>';
...
// echo zen_draw_form('custom', 'easypopulate_4.php', 'id="custom"', 'get');
  ...
  <?php
    ...
    echo zen_cfg_pull_down_order_statuses(NULL, 'order_status');
		echo zen_draw_input_field('exportorder', EASYPOPULATE_4_ORDERS_DROPDOWN_EXPORT, ' style="padding: 0px; width:auto;"', false, 'submit');
		?>
    <br /><br />
    </div></form>
...
// $upload_dir = DIR_FS_CATALOG.$tempdir; // defined above
        if ($dirhandle = opendir($upload_dir)) {
          ...
          $filenames = array(
            ...
            "orders-full-ep"=>(defined("ORDERSEXPORT_LINK_SAVE1")?ORDERSEXPORT_LINK_SAVE1:ORDERS_EP_DESC),
            "orders-fullb-ep"=>(defined("ORDERSEXPORT_LINK_SAVE1B")?ORDERSEXPORT_LINK_SAVE1B:ORDERS_EP_DESC),
            "orders-noattribs-ep"=>(defined("ORDERSEXPORT_LINK_SAVE2")?ORDERSEXPORT_LINK_SAVE2:ORDERS_EP_DESC),
            "orders-onlyAttribs-ep"=>(defined("ORDERSEXPORT_LINK_SAVE3")?ORDERSEXPORT_LINK_SAVE3:ORDERS_EP_DESC)
          );
          ...
          foreach ((EP4_SHOW_ALL_FILETYPES != 'false' ? $filenames_merged : $filetypes) as $key => $val) {
            ...
            (isset($val)?:$val=array()); // to prevent PHP Warning: sizeof(): Parameter must be an array or an object that implements Countable
            $file_count = 0;
