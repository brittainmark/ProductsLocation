...
//			$filelayout[] = 'v_products_location';
//		}
		if (count($custom_fields) > 0) { // User Defined Products Fields
			foreach ($custom_fields as $field) {
			  if ($chosen_key != 'v_' . $field) {
...
//		if ($ep_supported_mods['ploc'] == true) { // Products Location Mod
//			$filelayout_sql .= 'pl.products_location as v_products_location,';
//		}
		...
		$filelayout_sql .= ' p.products_weight as v_products_weight,
			...
			.TABLE_PRODUCTS.' as p ';
//			if ($ep_supported_mods['ploc'] == true) { // Products Location Mod
//				$filelayout_sql .= ', '. TABLE_PRODUCTS_LOCATION.' as pl'; // Association of this data to the rest is not shown which will cause data pull issues...
//			}
