...
<div class="container">
    ...
    <table class="table table-striped">
        ...
        <tr class="dataTableHeadingRow">
            ...
            <?php
/*
 * BOF - Very Simple Product location product location heading
 */
            if (defined('VS_PRODUCTS_LOCATION_PACKING_DISPLAY') && VS_PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                ?>
            <th class="dataTableHeadingContent" align="left"><?php echo TABLE_HEADING_VS_LOCATION; ?></th>
            <?php
            }
/*
 * EOF - Very Simple Product location
 */
?>
        </tr>
        </thead>
        ...
        <?php
/*
 * BOF Very Simple Product location sort
 */
        if (defined('VS_PRODUCTS_LOCATION_PACKING_DISPLAY') && VS_PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {

            $vs_products_location=array();
            for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
                $vs_products_location[$i]["order"] = $i;
                $vs_products_location[$i]["model"] = $order->products[$i]['model'];
                $vs_products_location_query = $db->Execute("SELECT IFNULL(vs_products_location,'') AS vs_products_location FROM " . TABLE_VS_PRODUCTS_LOCATION . "
        								   WHERE products_id = '" . $order->products[$i]['id'] ."' LIMIT 1");
                $vs_products_location[$i]["location"]=$vs_products_location_query->fields['vs_products_location'];
            }
            if (defined('VS_PRODUCTS_LOCATION_SORT_DISPLAY') && VS_PRODUCTS_LOCATION_SORT_DISPLAY == 'true') $vs_products_location_sort = array_multisort(array_column($vs_products_location, 'location'), SORT_ASC, array_column($vs_products_location, 'model'), SORT_ASC, $vs_products_location);

        }
        for ($ii = 0, $n = sizeof($order->products); $ii < $n; $ii++) {
            $product_name = $order->products[$i]['name'];
            if (defined('VS_PRODUCTS_LOCATION_PACKING_DISPLAY') && VS_PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                $i=$vs_products_location[$ii]['order'];
            } else {
                $i=$ii;
            }
            /*
            ...
            <tr class="dataTableRow">
                ...
                <td class="dataTableContent">
                    ...
                    <?php
/*
 * BOF - Very Simple Product location display location
 */
                    if (defined('VS_PRODUCTS_LOCATION_PACKING_DISPLAY') && VS_PRODUCTS_LOCATION_PACKING_DISPLAY == 'true') {
                    ?>
                <td class="dataTableContent" align="left" valign="top">
                    <?php echo  $vs_products_location[$ii]["location"]; ?>
                </td>
                    <?php
                    }
                    // EOF - Very Simple Product location
